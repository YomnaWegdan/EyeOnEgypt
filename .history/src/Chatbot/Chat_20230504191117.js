...
const Chat = () => {
  ...
  const [currentMessage, setCurrentMessage] = useState(    JSON.parse(window.localStorage.getItem("chatbot_messages"));  ||'')
 ...
 useEffect(() => {
    localStorage.setItem('chatbot_messages', JSON.stringify(message));
  },[message])
  useEffect(() => {
    JSON.parse(window.localStorage.getItem("chatbot_messages"));
  }, []);
