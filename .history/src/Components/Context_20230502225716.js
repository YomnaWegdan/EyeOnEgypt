...
const ContextProvider = ({children}) =>{
  ...
  const [Egyptology, setForYou] = useState([]);
  const [Entertaining, setTopPlaces] = useState([]);
  const [ReligiousTourism, setMostPopular] = useState([]);
  const [MedicalTourism, setMostPopular] = useState([]);
