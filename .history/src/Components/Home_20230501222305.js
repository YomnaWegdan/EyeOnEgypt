...
<BsSearch />
   ...
   {/*       <div className='dropdown'>{data.filter((item)=>{
          ...
          */}
          ...
          <NavDropdown  >
          {data.filter((item)=>{
            const searchTerm = name.toLowerCase()  || name.charAt(0).toUpperCase() || name.toUpperCase() ;;
            const fullName = item.name.toLowerCase()  || item.name.charAt(0).toUpperCase() || item.name.toUpperCase() ;
            return searchTerm && fullName.startsWith(searchTerm)
          }).map((item)=>(
            <div onClick={()=> onSearch(item.name)} className='dropdown-row'>
