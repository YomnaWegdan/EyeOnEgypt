...
const SignUp = () => {
  ...
  return (
    ...
    <Form   className='loginForm  ' style={{ marginLeft:'60px' , marginRight:'60px' , marginTop:'60px'  }}>
    ...
    <BsLock style={{position:'absolute' , marginTop:'11px' , marginLeft:'6px', color:'#644F0A'}}/>
        ...
        <Form.Control type="password" placeholder="Password" className='px-4' onChange={(e)=> setPassword(e.target.value)} value={password}/>
                ...
                <Button type="submit" className='btn w-100 mb-5' style={{ background: 'orange',color:'#644F0A' , border: 'none'}}> <NavLink style={{ color: '#644F0A', textDecoration:'none'}} to="/">Sign Up</NavLink></Button>

                <div style={{borderColor:'black' , color:'#644F0A'}} className='mb-4 text-center  '>Already have an account <NavLink  className="nav-link d-inline-block" to="/login">Sign up</NavLink> </div>

                <div className='mb-4 text-center d-inline-block' style={{ color:'#644F0A'}}>Already have an account <NavLink className="nav-link" to="/login">/login/login</NavLink></div>
